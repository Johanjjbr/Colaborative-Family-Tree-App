...
export function FamilyProvider({ children }: { children: ReactNode }) {
  ...
  const loadFamilyData = async () => {
    ...
    try {
      ...
      const [personsRes, relationshipsRes, activitiesRes] = await Promise.all([
        fetch(`${API_URL}/families/${loadedFamilyId}/persons`, {
          headers: { 'Authorization': `Bearer ${accessToken}` },
        }),
        fetch(`${API_URL}/families/${loadedFamilyId}/relationships`, {
          headers: { 'Authorization': `Bearer ${accessToken}` },
        }),
        fetch(`${API_URL}/families/${loadedFamilyId}/activities`, {
          headers: { 'Authorization': `Bearer ${accessToken}` },
        }),
      ]);
  ...
  const addPerson = async (
    ...
    // Fallback: no auth or no family → local state
    ...
    // Create the person on the server
    ...
    // Add relationships using the real new person ID
    if (parentId) {
